...
def wsi_prompt_selector(base_model, train_wsi_loader, prompt_lst, PPL_model = None, device = 'cuda:0'):
    
    base_model.eval()    
    with torch.no_grad():
        tokens = [conch_clip.tokenize(conch_clip.get_tokenizer(), prompts).to(device) for prompts in prompt_lst]
