...
def patch_selector(label_model, label_type, dataset_name, vision_feats, wsi_label, logits_thd = 0., text_embedding = None, device='cuda:0'):
    ...
    if label_type == 'zeroshot':
            ...
            with torch.no_grad():
                ...
                for prompts in prompts_sublist:
                    ...
                    for prompt in prompts:
                        tps, tpsd = musk_utils.xlm_tokenizer(prompt, musk_tokenizer, max_len=100)
                    ...
                    tokenized_pads = torch.cat(tokenized_pads)
                    tokens.append(tokenized_prompts.to(device))
                    paddings.append(tokenized_pads.to(device))
                ...
                manual_text_features = [torch.mean(label_model(text_description=token_input, padding_mask=pad_input)[1],dim = 0).cpu().numpy() for token_input,pad_input in zip(tokens, paddings)]
        ...
        logits = vision_feats.squeeze(0).float() @ text_features.t().float()
    else:
        label_model.eval()
        with torch.no_grad():
...
def wsi_prompt_selector(musk_model, musk_tokenizer, train_wsi_loader, prompt_lst, PPL_model = None, device = 'cuda:0'):
    
    # musk_model = timm.create_model("musk_large_patch16_384")
    # musk_utils.load_model_and_may_interpolate("hf_hub:xiangjx/musk", musk_model, 'model|module', '', local_dir = params.MUSK_PATH)
    ...
    with torch.no_grad():
        ...
        for prompts in prompt_lst:
            ...
            tokens.append(tokenized_prompts.to(device))
            paddings.append(tokenized_pads.to(device))
        text_features = [musk_model(text_description=token_input, padding_mask=pad_input)[1] for token_input, pad_input in zip(tokens, paddings)] #(class, num_template*num_classname)
    ...
    for idx in range(prompt_num): # random select 1000 prompt classifiers
        ...
        if PPL_model is not None:
            PPL_model.eval()
            with torch.no_grad():
                prompts, prompts_pad = PPL_model.prompt_learner()
                ...
                encoder_out = PPL_model.model.musk_model.beit3.encoder(src_tokens=None,
                                                                    encoder_padding_mask=prompts_pad.to(device),
                                                                    attn_mask=None,
                                                                    token_embeddings=prompts.to(device),
                                                                    multiway_split_position=0,
                                                                    incremental_state=None,
                                                                    positions=None,
                )
                token_text_features = encoder_out['encoder_out'][:,0,:]
                token_text_features = PPL_model.model.language_head(token_text_features)
