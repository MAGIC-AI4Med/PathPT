...
from PathPL_model_MUSK import OriginMUSK, CustomMUSK, PPLMUSK
...
from wsi_selecter_MUSK import patch_selector, wsi_prompt_selector
import json
...
def multiple_trains_and_eval(cfg, 
    ...
    for i in range(repeats):
        ...
        # origin_model = CustomMUSK(cfg, classnames_list, musk_model, musk_tokenizer, device, param, vfeat_dim=1024)
        ...
        zeroshot_train_dice = evaluation.draw_mask(zero_shot_model, train_wsi_loader, device=device, model_type='zeroshot', save_mask=True, save_path=f'mask/{param["dataset_name"]}/{param["log_name"]}/train')
        zeroshot_val_dice = evaluation.draw_mask(zero_shot_model, val_wsi_loader, device=device, model_type='zeroshot', save_mask=True, save_path=f'mask/{param["dataset_name"]}/{param["log_name"]}/val')
...
def model_init(cfg, classnames_list, musk_model, musk_tokenizer, device, param, vfeat_dim):
    model = PPLMUSK(cfg, classnames_list, musk_model, musk_tokenizer, device, param, vfeat_dim)
    ...
    for i in range(epoch):
        ...
        with torch.no_grad():
            ...
            if isinstance(metric,dict):
                ...
                train_dice = evaluation.draw_mask(model, train_wsi_loader, device=device, model_type='fewshot-mil', save_mask=(epoch==i+1),save_path=f'mask/{param["dataset_name"]}/{param["log_name"]}/train')
                val_dice = evaluation.draw_mask(model, val_wsi_loader, device=device, model_type='fewshot-mil', save_mask=(epoch==i+1),save_path=f'mask/{param["dataset_name"]}/{param["log_name"]}/val')
