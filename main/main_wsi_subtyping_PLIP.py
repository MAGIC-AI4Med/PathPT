...
from PathPL_model_PLIP import OriginPLIP, PPLPLIP
...
def multiple_trains_and_eval(cfg, 
    ...
    for i in range(repeats):
        ...
        train_dataset = WSI_Data(dataset_path=param['dataset_path'], 
                                feature_path = param['plip_feature_root'],
                                train_path = param["train_dir_path"],
                                fold = i,
                                patch_num=param['patch_num'],
                                state='train',val_kwargs=val_kwargs,)
        ...
        val_dataset = WSI_Data(dataset_path=param['dataset_path'], 
                        ...
                        fold = i,
                        state='val',
                        train_info = train_info,val_kwargs=val_kwargs,)
        ...
        test_wsi_dataset = WSI_Data(dataset_path=param['dataset_path'], 
                                feature_path = param['plip_feature_root'],
                                fold = i,
                                state='test',val_kwargs=val_kwargs,)
...
def train_anno(cfg, 
    ...
    ## first step
    init_model, optimizer, scheduler = model_init(cfg, classnames_list, base_model, base_tokenizer, device, param, vfeat_dim = 512)
...
def model_init(cfg, classnames_list, base_model, base_tokenizer, device, param, vfeat_dim):
    model = PPLPLIP(cfg, classnames_list, base_model, base_tokenizer, device, param, vfeat_dim)
...
def train_one_step(epoch, model, device, param, train_loader, train_wsi_loader, val_wsi_loader, label_model, label_type, optimizer, scheduler, metric, test_loader, selected_prompt_embedding = None, enable_pseudo = True):
    for i in range(epoch):
        ...
        for step, (data, _, label, _) in progress_bar:
            ...
            patch_labels = wsi_selecter_PLIP.patch_selector(label_model=label_model,
        ...
        if 'wsi_bacc' in val_result_dict:
            ...
            logging.info(f"Epoch {i} loss: {running_loss.item()*accumulation_steps / len(train_loader):.4f}, LR: {current_lr:.8f}, patch:{patch_rounded}, wsi:{wsi_rounded}, ensemble:{ensemble_rounded}, train_dice:{train_dice:.4f}, val_dice:{val_dice:.4f}")
        else:
            patch_result = np.array([val_result_dict['patch_bacc'],val_result_dict['patch_wf1']])
            patch_rounded = np.around(patch_result, decimals=4).tolist()
